...
delete from SOLDESK_USER_TB ; 
drop table SOLDESK_USER_TB cascade constraints;
...
insert into SOLDESK_USER_TB(u_email, u_pw, u_name, u_birth, u_gender)
values('test@test4.com', 'aaaa1111!', 'testman', '19911011','male');
insert into SOLDESK_USER_TB(u_email, u_pw, u_name, u_birth, u_gender)
values('test@test5.com', 'aaaa1111!', 'testman', '19911011','male');
insert into SOLDESK_USER_TB(u_email, u_pw, u_name, u_birth, u_gender)
values('test@test6.com', 'aaaa1111!', 'testman', '19911011','male');

-- 어드민 계정 생성
insert into SOLDESK_USER_TB(u_email, u_pw, u_name, u_birth, u_gender, u_admin)
values('admin@admin.com', 'aaaa1111!', '관리자', '19991111','male', 1);


-- 자유 게시판 테이블 생성
create table SOLDESK_BOARD_TB(
    ...
    constraint fk_b_email foreign key(b_email) references SOLDESK_USER_TB (u_email) on delete cascade
);
create sequence SOLDESK_BOARD_TB_SEQ;
drop sequence SOLDESK_BOARD_TB_SEQ;

INSERT INTO SOLDESK_BOARD_TB (b_no, b_email, b_title, b_writer, b_content, b_indate, b_count, boardGroup, boardSequence, boardLevel, boardAvailable)
SELECT nvl(max(b_no)+1, 1), 'test@test.com', '게시판연습', '홍길동', '반갑습니다', sysdate, 0, NVL(MAX(boardGroup) + 1, 0), 0, 0, 1
FROM SOLDESK_BOARD_TB;
INSERT INTO SOLDESK_BOARD_TB (b_no, b_email, b_title, b_writer, b_content, b_indate, b_count, boardGroup, boardSequence, boardLevel, boardAvailable)
